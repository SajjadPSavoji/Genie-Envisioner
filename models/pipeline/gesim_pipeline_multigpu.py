...
class GeSimCosmos2Pipeline(DiffusionPipeline):
    ...
    @property
    def transformer_device(self):
        """Returns the device of the transformer (diffusion model) for multi-GPU support."""
        return next(self.transformer.parameters()).device

    @property
    def vae_device(self):
        """Returns the device of the VAE for multi-GPU support."""
    ...
    def _get_t5_prompt_embeds(
        ...
        # ALWAYS use text encoder's actual device for encoding (multi-GPU fix)
        ...
        # Use text encoder device for attention mask
        prompt_attention_mask = text_inputs.attention_mask.bool().to(text_encoder_device)
        ...
        # Use text encoder device for input_ids
        prompt_embeds = self.text_encoder(
            text_input_ids.to(text_encoder_device), attention_mask=prompt_attention_mask
        ).last_hidden_state
        prompt_embeds = prompt_embeds.to(dtype=dtype, device=text_encoder_device)
    ...
    def encode_prompt(
        ...
        # Move embeddings to transformer device for multi-GPU support
    ...
    def infer(
        ...
        **kwargs,
        ...
        # Move scheduler sigmas and timesteps to transformer device for multi-GPU
        transformer_device = self.transformer_device
        self.scheduler.sigmas = self.scheduler.sigmas.to(transformer_device)
        ...
        video = video.to(device=self.vae_device, dtype=vae_dtype)  # Use VAE device for encoding
        ...
        # Use transformer device for multi-GPU
        sigma_conditioning = torch.tensor(sigma_conditioning, dtype=torch.float32, device=self.transformer_device)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # Move scheduler variables to transformer device for multi-GPU
                transformer_dev = self.transformer_device
                c_in = (1 - current_t).to(transformer_dev)
                c_skip = (1 - current_t).to(transformer_dev)
                c_out = (-current_t).to(transformer_dev)
                ...
                # Move current_sigma to transformer device
                noise_pred = (latents - noise_pred) / current_sigma.to(latents.device)
        ...
        if not output_type == "latent":
            ...
            video = self.vae.decode(latents.to(self.vae_device, self.vae.dtype), return_dict=False)[0]  # Move to VAE device
    ...
    def prepare_latents(
        ...
        # Move init_latents to transformer device for multi-GPU support
        transformer_device = self.transformer_device
        init_latents = init_latents.to(transformer_device)

        latents_mean = (
            torch.tensor(self.vae.config.latents_mean).view(1, self.vae.config.z_dim, 1, 1, 1).to(transformer_device, dtype)
        )
        latents_std = (
            torch.tensor(self.vae.config.latents_std).view(1, self.vae.config.z_dim, 1, 1, 1).to(transformer_device, dtype)
        )
        ...
        # Use transformer device for latents in multi-GPU setup
        if latents is None:
            latents = randn_tensor(shape, generator=generator, device=transformer_device, dtype=dtype)
        else:
            latents = latents.to(device=transformer_device, dtype=dtype)
