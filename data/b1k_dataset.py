...
class B1KDataset(Dataset):
    ...
    def get_batch(self, idx):
        ...
        # Get normalization statistics with correct dimensions
        action_mean, action_std = self.get_action_bias_std(domain_name, expected_dim=action.shape[1])
        state_mean, state_std = self.get_action_bias_std(domain_name + "_state", expected_dim=state.shape[1])
        ...
        if self.action_type == "absolute":
            ...
            delta_mean, delta_std = self.get_action_bias_std(domain_name + "_delta")
            ...
            # TODO: Identify and preserve gripper/binary action dimensions (like LeRobot does)
            # For B1K's 23-dim actions, determine which dimensions should remain absolute (not delta)
            # Example from LeRobot: delta_action[:, 6] = action_last[:, 6]  # left gripper
            action = (delta_action - delta_mean) / delta_std
        elif self.action_type == "relative":
            # Relative action type is not supported for B1K since actions (23 dims)
            # and states (256 dims) have different dimensions
            raise ValueError(
                f"Action type 'relative' is not supported for B1K dataset. "
                f"Actions have {action.shape[1]} dimensions but states have {state.shape[1]} dimensions. "
