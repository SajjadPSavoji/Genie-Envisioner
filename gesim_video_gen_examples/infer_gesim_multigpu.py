...
def prepare_model(args, dtype=torch.bfloat16, device="cuda:0", multi_gpu=False):
    # If multi_gpu, split: text_encoder+vae on cuda:0, diffusion_model on cuda:1
    device_text_vae = "cuda:0" if multi_gpu else device
    device_diffusion = "cuda:1" if multi_gpu else device
    print(f"Multi-GPU mode: {multi_gpu} | VAE+TextEncoder: {device_text_vae} | Diffusion: {device_diffusion}")
    ...
    text_encoder = text_encoder.to(device_text_vae, dtype=dtype).eval()
    ...
    if getattr(args, 'vae_path', False):
        vae = load_vae_models(vae_class, args.vae_path).to(device_text_vae, dtype=dtype).eval()
    else:
        vae = load_latent_models(vae_class, args.pretrained_model_name_or_path)["vae"].to(device_text_vae, dtype=dtype).eval()
    ...
    ### Load Diffusion Model (on separate GPU if multi_gpu)
    ...
    ).to(device_diffusion, dtype=dtype)
...
def infer(
    ...
    seed=42, device="cuda", default_fps=30, multi_gpu=False
):
    ...
    tokenizer, text_encoder, vae, diffusion_model, scheduler, pipe = prepare_model(args, device=device, multi_gpu=multi_gpu)
...
def args_parser():
    ...
    parser.add_argument('--multi_gpu', action='store_true', help='Use both GPUs (text_encoder+vae on cuda:0, diffusion on cuda:1)')
